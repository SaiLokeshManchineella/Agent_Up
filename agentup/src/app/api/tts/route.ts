...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.error('OpenAI TTS also failed, trying Cartesia multilingual fallback:', openaiError);
      
      // Third tier: Cartesia multilingual model (slower than sonic-2, but robust for failsafe)
      if (CARTESIA_API_KEY) {
        try {
          const response = await fetch('https://api.cartesia.ai/tts/bytes', {
            method: 'POST',
            ...
            body: JSON.stringify({
              model_id: 'sonic-multilingual', // Multilingual high-stability model
              transcript: text,
              voice: {
                mode: 'id',
                id: CARTESIA_VOICE_ID,
          ...
          console.error('Final Cartesia fallback failed:', mCallError);
        }
      }
    }
